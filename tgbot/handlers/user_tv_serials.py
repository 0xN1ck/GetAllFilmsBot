...
async def get_tv_serial(callback: CallbackQuery, kp_id, current_page, translate_id, quality, **kwargs):
    pass
    #data = await get_tv_serials(callback, kp_id)
...
async def navigate(call: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    levels = {
        "0": list_tv_serials,  # Отдаем категории
